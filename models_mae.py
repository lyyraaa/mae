...
class MaskedAutoencoderViT_SpectralGPTLoss(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        loss2 = (pred.reshape(B, c, h*w, pc*ph*pw).sum(1) - target.reshape(B, c, h*w, pc*ph*pw).sum(1)) ** 2
...
class MaskedAutoencoderViT_SmoothL1Loss(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        ln_mask = abs_diff >= beta # mask is 1 where difference is big
        ln0 = (0.5 * (pred - target) ** 2) / beta
        ln1 = abs_diff - 0.5 * beta
        loss = ln0 * (~ln_mask) + ln1 * (ln_mask)
